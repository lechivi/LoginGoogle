...
using UnityEditor;
#endif
public partial class CheatTool : MonoBehaviour
{
	private static CheatTool _instance;
	public static CheatTool Instance
	{
		get
		{
			if (_instance == null)
			{
				_instance = FindFirstObjectByType<CheatTool>();
				if (_instance == null)
				{
					GameObject go = new GameObject("CheatTool");
					_instance = go.AddComponent<CheatTool>();
	...
	public void TakeScreenshot()
	{
		...
		string nameFile = "Screenshot_" + Screen.width + "x" + Screen.height + " - " + unixTimeStamp + " - {0}_{1}_{2}" + ".png";
		nameFile = string.Format(nameFile, DateTime.Now.Day,
